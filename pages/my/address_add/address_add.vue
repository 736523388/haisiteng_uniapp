<template>
	<view>
		...
		<view v-if="loaded" class="container bg-white margin-top-xs padding-sm" style="border-radius: 20rpx;overflow: hidden;">
			<view>
				<view class="flex align-center padding-tb-sm padding-lr" style="width: 650rpx;">
				...
				<block v-if="show_default">
					<view style="margin: 0;">
						<u-line></u-line>
					</view>
					<view class="flex align-center" style="width: 650rpx;padding: 20rpx 30rpx;">
						<view style="width: 180rpx;flex: 1;">设置默认</view>
						<view style="margin-left: auto;">
							<switch :checked="is_default" color="#f56c6c" style="transform:scale(0.7)"
...
<script>
	...
	export default {
		...
		computed: {
			...mapState({
				user_address_list: state => state.user_address.address_list
			})
		},
		methods: {
			onDefaultChange(e) {
			...
			async loadData() {
				try {
					...
					if (this.id) {
						const res = await axios.get('/api/v1/user/address/' + this.id)
						if (res.code !== 1) {
						...
						this.show_default = res.data.is_default !== 1
					}
					this.loaded = true
				} catch (error) {
					console.log(error)
			...
			deleteAddress() {
				uni.showModal({
					...
					success: res => {
						//  点击确定
						if (res.confirm) {
							axios.delete('/api/v1/user/address/' + this.id).then(ref => {
								if (ref.code === 1) {
			...
			submit() {
				...
				this.loading = true
				axios.post('/api/v1/user/address', {
					id: this.id,
					name: this.name,
					phone: this.phone,
					...
					if (res.code === 1) {
						...
						this.$store.commit('user_address/update_user_address_list', {
							address_list: [...this.user_address_list, res.data]
						})
						setTimeout(() => {
							if (this.change !== '') {
