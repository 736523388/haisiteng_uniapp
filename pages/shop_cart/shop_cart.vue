<template>
	<view class="main">
		...
		<block v-if="is_login && loaded && (carts.length > 0 || invalid.length > 0)">
			<view class="flex justify-between align-center bg-white margin-bottom-sm padding-sm">
				...
				<view v-if="carts.length > 0" @click="showRemoveBtn=!showRemoveBtn"><text>{{showRemoveBtn ? '完成':'编辑'}}</text></view>
			</view>
			<view v-for="(item,index) in carts" :key="item.id" class="bg-white flex align-center padding-sm"
				style="border-bottom: 1px solid #F3F4F6;" @click="selectList" :data-index="index">
				...
				<view class="flex flex-direction justify-between" style="height: 180rpx;width: 100%;">
					<view class="goods-card-title van-multi-ellipsis--l2">
						<text>{{item.goods_title}}</text>
						<text v-if="item.stock < 1" class="text-xxs text-red padding-left-xs">库存不足</text>
					</view>
					<view class="padding-left-sm text-xs">{{item.goods_spec_alias}}<text class="margin-left-xs" style="color: rgb(244, 63, 94);">仅剩{{item.stock}}件</text></view>
					<view class="goods-card-price">
						<view>
							<view>
								<text class="goods-card-price-striking" style="font-size: 20rpx;">¥</text>
						...
						<view @click.stop>
							<u-number-box v-model="item.goods_number" :asyncChange="true"
								@change="valChange(index, $event)" :min="0" :max="item.stock" integer :button-size="24"></u-number-box>
						</view>
					</view>
				</view>
			</view>
			<view class="bg-white margin-top-sm" v-if="invalid.length > 0">
				...
				<view v-for="(item,index) in invalid" :key="item.id" class="bg-white flex align-center padding-sm"
					...
					<view class="margin-right-sm">
						<!-- <u-icon name="checkmark-circle-fill" color="#000030" v-if="item.selected"></u-icon> -->
						<!-- <u-icon name="checkmark-circle" color="#afafaf" v-else></u-icon> -->
...
<!-- 						<u-icon name="checkmark-circle-fill" color="#000030" v-if="selectAllStatus"></u-icon>
						<u-icon name="checkmark-circle" color="#afafaf" v-else></u-icon> -->
				...
				<view class="padding-right flex justify-end align-center" style="margin-left: auto;">
					<view v-show="!showRemoveBtn">
						<text>合计:<text class="text-xl text-red">￥{{totalPrice}}</text></text>
					</view>
					<view v-show="!showRemoveBtn">
					...
					<view v-show="showRemoveBtn">
						<u-button v-show="showRemoveBtn" @click.native="removeGoodsFromCart"
							custom-style="width: 100rpx;margin-left: 12rpx;" size="small" type="primary"
							text="删除"></u-button>
					</view>
				...
				<view class="order-icon" @click='topay'>
					<text>去结算</text>
				</view> -->
			</view>
		</block>
		<view class="bg-white" style="height: 100%;" v-if="loaded && ((carts.length == 0 && invalid.length == 0) || !is_login)">
